...
public class AzedisBot implements Bot{
	...
	// The public API of YourTeamName must not change
		...
		//Decides the minimum probability required for an attack
		private double minProbability = 0.5;
		//Markov Chains for the RISK Board Game-> matrix that gives the probabilites for attacks for armies greater than 10
		...
		AzedisBot (BoardAPI inBoard, PlayerAPI inPlayer) {
			board = inBoard;	
			player = inPlayer;
			// put your code here
			myId = getMyId();
		...
		//private classes and functions added to give the Bot extra functionalities and improvements for how it will play
		...
		//class for the countries and attacking using the Bot
		class Country {
			public int index;
			public String name;
		...
		//class for fortifying the countries 
		class Fortify{
			int armyDonator;
			int receiver;
			...
			public Fortify(Attack attack){
				armyDonator = attack.aID;
				receiver = attack.dID;
			}

			public int numTroops(){
				if(board.getNumUnits(armyDonator) == 2){
					return 1;
				}
				return (board.getNumUnits(armyDonator)/3)*2 -1;
			}

			public Boolean isPossible(){
				if(board.getNumUnits(armyDonator) > 1 && board.isConnected(armyDonator, receiver)){
					return true;
				}
				return false;
			}
			...
			public String toString(){
				return GameData.COUNTRY_NAMES[armyDonator] + "(" + board.getNumUnits(armyDonator) + ") " +
						"--(" + numTroops() + ")--> " +
